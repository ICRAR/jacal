...
int JacalBPCalibrator::run() {
  ...
  if (this->isWorker) {
      ...
      if (this->isParallel) {
          ...
          casacore::IPosition start(2, this->itsBeam, (this->itsChan)*allocation);
          casacore::IPosition stop(2, this->itsBeam, (this->itsChan+1)*allocation-1);

          ASKAPLOG_INFO_STR(logger,"Allocation of " << allocation << " starts at " << start << " stop " << stop);

          itsWorkUnitIterator.init(casacore::IPosition(2, nBeam(), nChan()), start, stop);
          //itsWorkUnitIterator.init(casacore::IPosition(2, nBeam(), nChan()), 1, this->itsChan);
  ...
  if (this->isMaster && this->isParallel) { // the master rank has to wait for the workers
      ...
      for (std::vector<int>::const_iterator it =  this->itsModelInputs.begin(); it != this->itsModelInputs.end(); ++it) {
        ...
        const casacore::uInt numberOfWorkUnits = nBeam() * nChanPerRank();
...
//     for (casacore::uInt chan ; chan < nChan(); ++chan ) {
//
...
std::pair<casacore::uInt, casacore::uInt> JacalBPCalibrator::currentBeamAndChannel() const
{
...
void JacalBPCalibrator::calcOne(const std::string& ms, const casacore::uInt chan, const casacore::uInt beam)
{
  casacore::Timer timer;
  timer.mark();
