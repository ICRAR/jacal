...
{ # Wait for daliuge to start, but timeout after 10 seconds
    ...
    TIMEOUT=10
    dlg unroll-and-partition -L ./askap_SpectralLine.json -a metis | dlg map -N localhost,localhost,localhost,localhost,localhost,localhost,localhost,localhost,localhost -i 8 | dlg submit -H localhost -p 8001
