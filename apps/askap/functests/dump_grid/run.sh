...
{ # Wait for daliuge to start, but timeout after 10 seconds
    ...
    TIMEOUT=10
    dlg unroll-and-partition -L ./dump_grid.json -a metis | sed 's/DynlibApp/DynlibProcApp/g' | dlg map -N localhost,localhost,localhost,localhost,localhost,localhost,localhost,localhost,localhost -i 1 | dlg submit -H localhost -p 8001
