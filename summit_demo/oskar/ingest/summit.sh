...
rm summit_simulator_erf
rm summit_imager_erf
...
for idx in $(seq 0 $uidx) ; do
    echo "1 : {host: 1; cpu: {${idx}}; gpu :{${idx}} } : app ${idx}" >> summit_simulator_erf
    echo "1 : {host: 1; cpu: {${idx}}; gpu :{${idx}} } : app ${idx}" >> summit_imager_erf
done
